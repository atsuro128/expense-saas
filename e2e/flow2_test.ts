...
/**
 * CRS-071 詳細テスト: 元レポートからの再申請で添付ファイルがコピーされないことを API で確認する。
 *
 * 元レポートに添付ファイルが無いケースで、新規レポートの items[].attachments が
 * 空配列で返ること（reference_report_id 経由のコピーで添付が引き継がれないこと）を検証する。
 * UI 経由ではなく API 経由でレポート状態を準備してから検証する。
 */
test('CRS-071: 元レポート再申請時に新規レポートの添付配列が空であることを API で確認する', async ({ request }) => {
