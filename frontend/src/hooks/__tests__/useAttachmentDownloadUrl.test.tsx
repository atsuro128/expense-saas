// useAttachmentDownloadUrl Hook のユニットテスト。
...
import { useAttachmentDownloadUrl } from '../useAttachmentDownloadUrl';
...
describe('useAttachmentDownloadUrl', () => {
  ...
  // ATT-FE-033: enabled: false の検証 — 初期レンダリング時に fetch が呼ばれないこと。
  it('ATT-FE-033: enabled: false のため初期レンダリング時に fetch が呼ばれない', () => {
  ...
  // ATT-FE-033b: refetch() 後のみ fetch が呼ばれること。
  it('ATT-FE-033b: refetch() を呼ぶと正しい URL で GET リクエストが送られ、署名付き URL が返る', async () => {
    const mockAccess = {
      data: {
    ...
    // 正しい URL（/download）で fetch が呼ばれること。
    const calledUrl = (globalThis.fetch as ReturnType<typeof vi.fn>).mock.calls[0]?.[0] as string;
    expect(calledUrl).toContain(
      '/api/reports/report-001/items/item-001/attachments/att-001/download',
    );
    // レスポンスに url フィールドが含まれること（AttachmentAccess 形式）。
    expect(result.current.data?.data.url).toBe('https://s3.example.com/signed-url?token=abc123');
  });
  ...
  // ATT-FE-035: API が 403 FORBIDDEN を返すと isError=true になる。
  it('ATT-FE-035: API が 403 FORBIDDEN を返すと isError=true になる', async () => {
    globalThis.fetch = vi.fn().mockResolvedValueOnce({
      ok: false,
      status: 403,
      headers: { get: () => null },
