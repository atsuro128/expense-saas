...
// ATT-FE-041〜044, ATT-FE-061 に対応する。
// ATT-FE-061 は機能実装フェーズ（issue #108 対応）で green になる想定（AbortController 未実装のため FAIL）。
...
describe('useDeleteAttachment', () => {
  ...
  // ATT-FE-061: unmount 時に AbortController.abort() が呼ばれ、進行中の DELETE リクエストが中断される。
  // 機能実装フェーズ（issue #108 対応）で green になる想定（AbortController が未実装のため FAIL）。
  it('ATT-FE-061: aborts_delete_mutation_on_panel_close', async () => {
    // AbortController のグローバルスパイを設定する。
    const abortSpy = vi.fn();
    const originalAbortController = globalThis.AbortController;

    // AbortController をラップしてスパイする（useUploadAttachment テストと同一パターン）。
    ...
    const { result, unmount } = renderHook(() => useDeleteAttachment(), {
