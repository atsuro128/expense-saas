...
describe('AttachmentUploader 追加モード（ATT-FE-073, 074, issue #115/#129）', () => {
  ...
  //             issue #143: UI 構造が編集モードと同一（<ul><li> + <Button variant="text" color="error">削除</Button>）。
  it('ATT-FE-073: buffers_selected_file_in_local_state_in_add_mode', async () => {
    ...
    // 「保留中」バッジも表示されないこと（issue #143、案 A: 実装詳細を UI に露出しない）。
    ...
    // issue #143 UI 構造検証: 保留ファイル一覧が <ul><li> 構造であること（編集モードと同型）。
    ...
    // 削除ボタンが <Button variant="text" color="error">削除</Button> であること（<IconButton>× ではない）。
    // data-testid で特定し、ボタンテキストが「削除」であることを確認する。
    const deleteBtn = screen.getByTestId('pending-attachment-delete-0');
    expect(deleteBtn).toHaveTextContent('削除');
    // MUI Button variant="text" の場合は <button> 要素として描画される。
    expect(deleteBtn.tagName.toLowerCase()).toBe('button');

    // ファイルサイズが <span> 要素で表示されること（<Typography variant="caption"> ではない）。
