...
func TestListTenantMembers_Admin_OK(t *testing.T) {
	...
	// 前提: テナントAに Admin, Approver, Approver2, Member, Accounting, Member Empty の 6 ユーザーが登録済み（SeedFixtures による）
	...
	// 検証: data が配列で 6 件のメンバーが返ること
	// issue-166: テナント A 第二 Approver (UserApprover2ID) 追加により 5 件→6 件に変更。
	var resp struct {
		Data []struct {
			ID   string `json:"id"`
			Name string `json:"name"`
	...
	// data が配列であること（6 件）
	if len(resp.Data) != 6 {
		t.Errorf("data の件数: got %d, want 6 (body: %s)", len(resp.Data), rec.Body.String())
	}
