...
中小規模の組織を想定した SaaS 型の経費精算アプリケーション。MVP の機能境界は「経費レポートの作成・提出・承認・却下・支払完了」までの一連のワークフローを 4 ロール（Admin / Approver / Member / Accounting）の RBAC とテナント分離で制御することに絞り込んでいる。スコープ詳細は [`02_scope.md`](../dev-journal/deliverables/docs/02_scope.md) を参照。

### 何を示したいプロジェクトか

「動くものを作った」ではなく、**業務 SaaS で問われる設計判断と品質保証を一通り通せること** を示すことを目的としている。具体的には以下。

1. **マルチテナント設計**: 全クエリでの `tenant_id` 強制 + PostgreSQL RLS による二重保証（[ADR-0003](../dev-journal/deliverables/docs/30_arch/adr/0003-rls-tenant-isolation.md)）
2. **RBAC**: 4 ロール × ミドルウェア検証 × 所有権チェック（リソース所有者のみ操作可）
3. **業務フローの厳密実装**: 状態遷移（draft → submitted → approved → paid / rejected）をドメイン層で制御、不正遷移はエラー
4. **設計判断の記録**: 技術選定の判断根拠を ADR 7 件で明文化（[adr/](../dev-journal/deliverables/docs/30_arch/adr/)）
5. **テスト戦略**: テナント分離 / RBAC / 状態遷移を重点項目として明示し、ユニット〜E2E まで設計（[60_test/](../dev-journal/deliverables/docs/60_test/)）
6. **運用設計**: 構造化ログ・CloudWatch メトリクス・ヘルスチェック・アラート方針を含めた運用設計（[ADR-0005](../dev-journal/deliverables/docs/30_arch/adr/0005-monitoring-logging.md)）
...
| 領域 | 採用技術 | 選定理由（要約） |
|---|---|---|
| バックエンド | Go 1.24 / Chi / sqlc / pgx | 型安全な SQL（sqlc） + 軽量ルータ。詳細は [ADR-0001](../dev-journal/deliverables/docs/30_arch/adr/0001-tech-stack.md) |
| 認証 | JWT (RS256) + Argon2id | 公開鍵検証で API ステートレス化。Argon2id は OWASP 推奨。詳細は [ADR-0006](../dev-journal/deliverables/docs/30_arch/adr/0006-jwt-signing-algorithm.md) |
| フロントエンド | React 19 / TypeScript / Vite 6 / MUI 6 / TanStack Query | サーバー状態は TanStack Query で集約 |
| DB | PostgreSQL 16 + RLS | Shared DB + tenant_id 方式、RLS でセーフティネット。詳細は [ADR-0002](../dev-journal/deliverables/docs/30_arch/adr/0002-multi-tenant.md) / [ADR-0003](../dev-journal/deliverables/docs/30_arch/adr/0003-rls-tenant-isolation.md) |
| オブジェクトストレージ | S3 (本番) / MinIO (ローカル) | 領収書ファイル。署名付き URL 配信（テナントプレフィックス分離） |
| インフラ | AWS（CloudFront / ALB / EC2 t3.micro / RDS / S3 / CloudWatch / SSM） | Free Tier 範囲のポートフォリオ構成。詳細は [ADR-0004](../dev-journal/deliverables/docs/30_arch/adr/0004-infra.md) / [ADR-0007](../dev-journal/deliverables/docs/30_arch/adr/0007-cloudfront-https.md) |
...
詳細は [`architecture.md`](../dev-journal/deliverables/docs/30_arch/architecture.md) / 図一覧は [`diagrams.md`](../dev-journal/deliverables/docs/30_arch/diagrams.md) を参照。

### セキュリティ多層防御

ネットワーク（CloudFront ヘッダ検証）→ トランスポート（HSTS / CORS）→ 認証（JWT RS256）→ 認可（RBAC + 所有権）→ データアクセス（`tenant_id` 強制 + RLS）→ 出力制御（テナント越境は 404）の 6 層で実装している。詳細は [`security.md`](../dev-journal/deliverables/docs/50_detail_design/security.md) / [`authz.md`](../dev-journal/deliverables/docs/50_detail_design/authz.md) を参照。
...
| ADR | テーマ |
|---|---|
| [0001](../dev-journal/deliverables/docs/30_arch/adr/0001-tech-stack.md) | 技術スタック・主要ライブラリの選定理由 |
| [0002](../dev-journal/deliverables/docs/30_arch/adr/0002-multi-tenant.md) | マルチテナント方式（Shared DB + tenant_id） |
| [0003](../dev-journal/deliverables/docs/30_arch/adr/0003-rls-tenant-isolation.md) | RLS テナント分離方式の判断根拠 |
| [0004](../dev-journal/deliverables/docs/30_arch/adr/0004-infra.md) | インフラ選定（コンピュート / DB / ストレージ） |
| [0005](../dev-journal/deliverables/docs/30_arch/adr/0005-monitoring-logging.md) | 監視・ログ戦略 |
| [0006](../dev-journal/deliverables/docs/30_arch/adr/0006-jwt-signing-algorithm.md) | JWT 署名アルゴリズム選定 |
| [0007](../dev-journal/deliverables/docs/30_arch/adr/0007-cloudfront-https.md) | CloudFront による HTTPS 化 |

---

## 開発プロセス

要件定義 → 設計 → 実装 → テスト → デプロイ → UAT を段階的に進める計画駆動型のプロセスで進めている。各 Step の成果物・進捗・意思決定ログ・発見した issue は [`dev-journal/`](../dev-journal/) に一通り残してある。

| Step | 内容 | 主な成果物 |
|---|---|---|
| 1 | 要件定義 | [requirements.md](../dev-journal/deliverables/docs/10_requirements/requirements.md), [usecases.md](../dev-journal/deliverables/docs/10_requirements/usecases.md) |
| 2 | ドメイン設計 | [20_domain/](../dev-journal/deliverables/docs/20_domain/) |
| 3 | アーキテクチャ設計 | [architecture.md](../dev-journal/deliverables/docs/30_arch/architecture.md), [ADR](../dev-journal/deliverables/docs/30_arch/adr/) |
| 4 | 基本設計 | [40_basic_design/](../dev-journal/deliverables/docs/40_basic_design/) |
| 5 | 詳細設計（API / DB / 認可 / セキュリティ） | [50_detail_design/](../dev-journal/deliverables/docs/50_detail_design/) |
| 5.5 | UI コンポーネント設計 | [55_ui_component/](../dev-journal/deliverables/docs/55_ui_component/) |
| 6 | テスト設計 | [60_test/](../dev-journal/deliverables/docs/60_test/) |
| 7 | 運用設計 | [70_operations/](../dev-journal/deliverables/docs/70_operations/) |
...
進捗は [`progress-management/progress.md`](../dev-journal/progress-management/progress.md)、検出された issue とその扱いは [`issues/`](../dev-journal/issues/) を参照。

実装作業の効率化として、Claude Code（AI コーディングエージェント）と自作の開発フレームワーク（[`ai-dev-framework/`](../ai-dev-framework/)）を併用している。役割別エージェント（architect / designer / backend / frontend / test-implementer / reviewer 等）を切り替えながら開発を進める運用とした。設計判断・レビュー判定・コミット・マージ等の責任ある判断は人間が行い、AI は実装・調査・草案作成の労力削減手段として位置づけている。

---

## リポジトリ構成

本プロジェクトは 4 つのディレクトリで構成されている。

| ディレクトリ | 役割 |
|---|---|
| [`expense-saas/`](.) | プロダクト本体（実装コード） |
| [`dev-journal/`](../dev-journal/) | 開発プロセス記録（設計成果物・進捗・issue・ログ） |
| [`ai-dev-framework/`](../ai-dev-framework/) | AI 駆動開発フレームワーク（エージェント定義・ワークフロー・テンプレート） |
...
投入されるデータの詳細は [`test_strategy.md §4.2/4.3/4.4`](../dev-journal/deliverables/docs/60_test/test_strategy.md) を参照。
